...
vars <- c("M206","M202","M204","M205", "M209")
i <- all_vt_result$id %in% vars

boxplot(yield_lb ~ id, data = all_vt_result[i,],
        xlab = "Cultivar",
        ylab = "Yield (lbs/ac)")
@
...
\section{Evidence of yield decreases over time}
...
\begin{figure}
  ...
  mods <- lmList(yield_lb ~ (1|year) | id, data = all_vt_result[i,])
...
<<echo=FALSE>>=
meds_split <- split(all_vt_result[i,], paste(all_vt_result[i, "year"], all_vt_result[i,"site"]))
...
mod <- stan_glm(yield_lb ~ factor(year) + site + factor(year):site +
                ...
                data = all_vt_result[i,], prior = normal(), cores = 2L, iter = 1000)
...
plot(yield_lb ~ year, all_vt_result[i,],
     xlab = "Year", ylab = "Yield (lbs/ac)")

abline(lm(yield_lb ~ year, all_vt_result[i,]))
