...
compared to the preceding period. Yet, annual reports from the CA
...
text(20, 3500, bquote(.(round(fixef(yrs_fit)[2] * 1000, 1))~lbs~year^-1))

@ 
\caption{There is a small decrease in yield over the time since a
  cultivar was entered into the trial.}
\label{fig:yrs}
\end{figure}

As seen in Fig. \ref{fig:yrs}, there is a small decrease in yield
over time once a cultivar is released. This decrease is estimated to be
...
acre\textsuperscript{-1} since release.
...
text(1990, 3500, bquote(.(round(fixef(release_fit)[2] * 1000, 1))~lbs~year^-1))
@

\caption{There is an estimated increase in yield over time, but it is
  smaller than the previously estimated 54 lbs
...
invisible(sapply(tars, function(x){
    plot(yield_lb ~ year, data = mod_data[(mod_data$id == x & mod_data$site == "RES"),],
         xlim = c(1995, 2016), ylim = c(6000,12000),
         pch = 16, main = x,
         ylab = "",
         yaxt = "n",
         xlab = "Year")
...
@
\caption{The performance of three major cultivars at the RES site over
time.}
\label{fig:res_yrs}
\end{figure}
...
@
\caption{The performance of three major cultivars at the RES site
  since the year each cultivar was released.}
