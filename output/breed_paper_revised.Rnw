...
$k$, and lastly $\beta_{cultivar_k:year_j}$ is the interaction term
for cultivar $k$ in year $j$. This
...
kg\tsups{-1} ha\tsups{-1} yr) and does not provide
...
of decline overlapped). Differences between cultivars in the decline
...
ocean winds through the Delta ~\citep{espe16a}. These three cultivars
...
limited adoaptation range so is only grown in a limited area). The
variety cultivar M-206 was released in 2004 and is currently still the
most widely grown rice varietycultivar in California. At the time of
...
improvements in yield potential have not been made over the 33-yr
...
yr\tsups{-1}. The simpliest model (Model 2) in this study is similar
to the analyses of~\citet{Omar2014} and ~\citet{mckenzie2014}, and
results in similar findings; yields were estimated to increase by
\Sexpr{only_release_coef} kg ha\tsups{-1} yr\tsups{-1}
(Fig. \ref{fig:wo_erosion}), almost identical to estimates by
~\citet{mckenzie2014}.

The only way to reconcile the two apparently contradictory findings above
is that the yield of potential of cultivars decline over
time such that when newly released cultivars are compared
with the older cultivar (as shown by ~\citet{Omar2014},
~\citet{mckenzie2014}, and Fig. \ref{fig:wo_erosion}), yields of the
new cultivar are higher. Here we provide evidence that
cultivar yield does decline - on average at a rate of
\Sexpr{yrs_coef} kg ha\tsups{-1} yr\tsups{-1}
(Fig. \ref{RES_year}). ~\citet{Peng2010} also found that a cultivar's
yield potential declines over time and reported a 15\%
decline over a 30 yr period.  In this study, over a 30 year period and
a \Sexpr{yrs_coef} kg ha\tsups{-1} yr\tsups{-1} yield decline rate,
the yield decline would be approximately
\Sexpr{round((yrs_coef*-30)/mean(mod_data$yield_kg), 3)*100}\%. Given
the estimates here using the model which accounts for these declines
...
25 kg\tsups{-1} ha\tsups{-1} year\tsups{-1}.
...
time of \Sexpr{yrs_coef} Mg ha\tsups{-1} year\tsups{-1} found in this
...
yields. Yield potential can be defined as the yield of optimum
...
no substantial trends in growing season temperatures (either high,
low, or average temps) in the Sacramento Valley during the study
period, suggesting that older cultivars do not have lower yield
potential due to changing climatic factors. Our analysis
offers inconclusive evidence of an interaction between cultivar and
year (i.e., the CI for the interactions overlapped with each other and zero), visual examination suggests that cultivars may be
different in terms of rates of yield decline
...
\begin{comment}
It is striking that the estimated yield decline over time is similar
to that reported by \citet{Peng2010} over a similar time span (15\%
decline over roughly 30 years). This suggests the mechanisms
responsible could potentially be endemic to all rice production
systems. While efforts should be made to identify the cause of yield
decline over time, this study highlights both the need for maintenance
of plant breeding and the difficulty of increasing genetic yield
~\citep{espe2017} potential, especially in systems such as CA where
yield is approaching the physiological limit \citep{espe16a}.
...
\ref{fig:compare_yrs}) compared to M-202 released at the start of this
period. Although M-205 is not broadly adapted, it has
increase yield potential (Fig. \ref{fig:first_yrs} and its adoption (although limited) in the
...
explanations for this discrepancy in conclusions. First, \citet{mckenzie2014} did not
...
as that found by \citet{mckenzie2014}. Second, these yield decreases over time
...
could suggest the analysis lacks the statistical power to detect the
trend. Third, it
...
timing. By contrast, the investigation presented here used a single,
...
both the need for plant breeding to simply maintain current yields, as
well as the challenge of increasing genetic yield potential,
especially in systems such as CA where yield is approaching the
physiological limit. The goal of increasing yield may require the
yield performance of cultivars to be assessed earlier in the selection
process to avoid discarding higher yielding entries without full
consideration. We suggest here that historical benchmarks be used in
to evaluate promising rice cultivars in addition to side-by-side
comparisons to maintain perspective of the absolute yield gains being
made.
