...
yield. However, the yields of older cultivars declined at an estimated
rate of 29.3 kg ha\tsups{-1} year\tsups{-1} (90\% credible interval
-4.4 to -53.3) after initial selection. Once this effect was
...
\begin{equation}
\begin{split}
 yield_{ijk} = Intercept + (Rate_{yield\ decline}* {years\ since\
   release\ ik}) \\+ (Rate_{yield\ improvement}* {release\ year\ ik})\\ 
 + \beta_{year\ ij} + \beta_{cultivar\ ik} +
 \beta_{cultivar\ ik:year\ ij} + \epsilon \\
\end{split}
\end{equation}
...
decline}$ is rate of yield decline per ${years\ since\ release\ i}$
...
the $Rate_{yield\ decline\ i}$ term (Model 2). Lastly, a model was fit
...
most widely planted cultivar for two decades~\citep{mkenzie2014}. At
...
10.8 and 11.5 Mg ha\tsups{-1} (Fig. \ref{first_yrs}), with the
...
its release, M-206 was reported to yield \Sexpr{round(9350*1.12,1)} Mg
ha\tsups{-1} -- also similar to what we found based on our analysis
...
yr\tsups{-1}. Using a similar approach to~\citet{Omar2014} and
~\citet{mckenzie2014}, when the compared average yields for the period
2011--2016 of varieties cultivars released since 1982 (and still in
the variety trials from 2011-- 2016) was compared without accounting
for yield declines over time, yields were estimated to increase by
\Sexpr{only_release_coef} kg ha\tsups{-1} yr\tsups{-1}
(Fig. \ref{fig:wo_erosion}), almost identical to what
~\citet{mckenzie2014}.

The only way to reconcile the two apparently contradictory findings,
is that the yield of potential of varieties cultivars decline over
time such that when newly released varieties cultivars are compared
with the older variety cultivar (as shown by ~\citet{Omar2014},
~\citet{mckenzie2014}, and Fig. \ref{fig:wo_erosion}), yields of the
new cultivar variety are higher. Here we provide evidence that
cultivar varietal yield does decline - on average at a rate of
\Sexpr{yrs_coef} kg ha\tsups{-1} yr\tsups{-1}
(Fig. \ref{w_erosion}). ~\citet{Peng2010} also found that a cultivar’s
varieties yield potential declines over time and reported a 15\%
...
\Sexpr{round((yrs_coef*-30)/mean(mod_data$yield_kg), 3)*100}\%.
...
potential due to changing climatic factors. While our analysis
indicates that there is no interaction between cultivar and year (CI
includes zero), visual examination suggests that cultivars may be
...
declines over time. Quantifying changes in yield potential requires also
