...
\title{Rice yield improvement by plant breeding are offset by
  decreases over time}
...
to increase total production to meet growing demand, which has
necessitated harvesting more grain per unit land area. Historically,
...
yield \citep{Peng2010}. If continued improvements per unit land area
...
understand why grain yields have not continued to increase.

CA rice production represents one such rice production system. Rice
yields in CA during the period from 1920 -- 1990 increased rapidly, but
since 1990 have increased only slowly if at all (Fig. 1
\ref{fig:avg_yields}). However, continued improvement of grain yield
...
example, at the year of release (1984), cultivar 'M-202' averaged
9,600 lbs acre\textsuperscript{-1}. In 2015, the yield of 'M-209' was
between 9,000 -- 10,000 lbs acre\textsuperscript{-1}. We sought to better
understand how grain yields have increased in the breeders' fields
without a corresponding increase in yield in the farmers' fields. Two
competing hypotheses were tested: (1) Grain yields have not improved
since 1990, or (2) grain yield improvements have been offset by an
associated decrease in yield once a cultivar was selected.
...
\caption{Average rice yields over the period \Sexpr{min(CA_data$Year)}
  -- \Sexpr{max(CA_data$Year)} for California, USA. Rapid gains in
    ...
    annual gains since 1990 have been slower.}
...
plot_data = as.data.frame(tapply(mod_data$yield_lb[res] * 1.12,
                                 mod_data$id[res], mean))
plot_data$se = tapply(mod_data$yield_lb[res] * 1.12,
                      mod_data$id[res],
                      function(x)
                          sd(x)/sqrt(length(x)))
...
abline(a = (((fixef(RES_rel_fit)[1] * 1000) + 9000) * 1.12) -
       ...
       b = fixef(RES_rel_fit)[2] * 1000, col = "grey")
...
# text(0, 7000, bquote(.(round(fixef(RES_yrs_fit)[2] * 1000, 1))~lbs~year^-1))
...
plot_data = as.data.frame(tapply(mod_data$yield_lb[res] * 1.12,
                                 mod_data$yrs_in_trial[res], mean))
plot_data$se = tapply(mod_data$yield_lb[res] * 1.12,
...
abline(a = ((fixef(RES_yrs_fit)[1] * 1000) + 9000) * 1.12,
       b = fixef(RES_yrs_fit)[2] * 1000)
ints <- apply(pp, 2, quantile, p = c(0.05, 0.5, 0.95))
#polygon(x = c(yrs_in, rev(yrs_in)), y = c(ints[1,],rev(ints[3,])), col = rgb(0,0,0,0.2), border = NA)
text(0, 7000, bquote(.(round(fixef(RES_yrs_fit)[2] * 1000 * 1.12, 1))~kg~ha^-1~year^-1))
...
\begin{figure}
<<fig=TRUE, fig.width = 10, fig.height = 5, out.width = '80%', echo=FALSE>>=
tars <- c("M202","M206")
...
invisible(sapply(tars, function(x){
    plot(yield_lb*1.12 ~ yrs_in_trial,
         data = mod_data[(mod_data$id == x & mod_data$site == "RES"),],
