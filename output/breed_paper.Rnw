...
side-by-side comparisons between old and new rice varieties was
observed when the yield erosion was not considered. However, the
yields of older varieties eroded at an estimated rate of 29.3 kg
ha\tsups{-1} year\tsups{-1} (90\% credible interval -4.4 to -53.3)
after initial selection. Once this effect was considered, the yield
advantage of newer cultivars over old was uncertain (-3.3 kg
ha\tsups{-1} year\tsups{-1}, 90\% credible interval -36.1 to
31.5). These results highlight the importance of continuous crop
improvement and deployment of new cultivars as yield will decline over
time without it. To increase the genetic yield potential of rice in
the face of this phenomenon, higher yield targets are needed to
counter-act the decrease over time in the current cultivars.
...
having a Mediterranean climate, with long days, dry growing season,
and relatively free of pests and diseases, which lead to some of the
highest yields in the world~\citep{FAO2016}. Production in CA is
predominately focused on premium quality medium grain
...
with yields on average increasing by about 50 to 62 kg ha\tsups{-1} between
1999 and 2014.
...
announcements, based on yield performance in side-by-side variety
trails, data show that new varieties usually out yield the older
...
yield comparison tests will not reflect true yield increases.o
...
1969. The California Statewide Variety Trails evaluates current and
...
Esquon-Neerdobe clays with roughly 2\% soil organic matter
\citep{ssurgo} in the top 15cm.
...
3--4 times in a CRB design) were collected leading to a final data set
...
systems, for planting, rice seed is pre-germinated prior
...
There was agreement between the estimates from the Bayesian and
classical approaches.  The Bayesian estimates are presented here,
...
trials (\Sexpr{unique(mod_data[which.min(mod_data[,"year"]),"id"])}
...
kg ha-1 yield advantage of M-206 over M-202 can be attributed solely
...
When yield erosion was not corrected for in the analysis (i.e.,
...
include M-208, which had resistance to blast disease (scientific name)
...
at \Sexpr{round(mean(mod_data[,"yield_kg"]), 2)} Mg
...
years by an average of \Sexpr{yrs_coef} Mg ha\tsups{-1}, potentially
...
ambitious target of 0.5 Mg ha\tsups{-1} (approximately 5\%) increase
...
show an improvement in grain yield of 2\% per release, in which case
...
into account erosion in the current ``standard'' cultivars' yield
