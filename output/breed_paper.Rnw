...
been seen in farmers' fields. Side-by-side comparisons under yield
erosions will show ``false'' increases due to the older cultivar
decreasing, not due to increased genetic yield potential by the newer
cultivar. We confirmed an apparent increase in yield in side-by-side
comparisons between old and newer varieties when the yield erosion was
not considered. However, we demonstrate this apparent increase in
yield can be explained by the differences in the time since selection
between cultivars. The impact of yield erosion was estimated to be
29.3 kg ha\tsups{-1} year\tsups{-1} (90\% credible interal -4.4 to
-53.3) after initial selection. Once this effect was considered, the
apparent yield advantage of newer cultivars over old was uncertain
(-3.3 kg ha\tsups{-1} year\tsups{-1}, 90\% credible interval -36.1 to
31.5). This underscores the importance of breeding to maintain yield
and the challenges of increasing the genetic yield potential.
...
tested: (1) the genetic yield advantage has increased over the last
30+ years, and (2) the genetic yield advantage has not increased over
the last 30+ and the erosion of yield over
...
3 to 4 times the number of plots compared to any other location). For
these reasons, we focused our investigation on the RES to avoid
potential complications due to site, climate, and management
differences at the other 6 to 8 farmer-managed trials per year.
...
90\% of the planted area in CA. These cultivars are typcically planted
...
Plots at the RES are managed similar to University of California
...
the model was as above except omitting the $Rate_{yield\ erosion\ i}$ term.
...
The full model which corrected for both the year of release and yield
erosion was estimated to have better out-of-sample predictive accuracy
compared to reduced models containing only one predictor. As estimated
by the best model, there is no conclusive evidence that the yields of
medium grain rice cultivars selected and released by CCRRF's breeding
program have, on average, increased over the last 30 years. The
...
(Fig. \ref{fig:w_erosion}). Hence, the evidence here is consistent
with little to no increase in yield potential over the study
period. When yield erosion is not corrected for in the comparison
...
\Sexpr{only_release_CI[2]}) (Fig. \ref{fig:wo_erosion}), though this
estimate ignores the confounding effect of yield erosion over time and
performed worse in out-of-sample predictive accuracy.

After correcting for the other variables, some cultivars are estimated
to have greater yield compared to the overall average. The marginal
estimate for the performance of a cultivar averaged over the
uncertainty in the other parameters in the model (i.e., the random
effect) shows three cultivars with higher than average yields:
\Sexpr{paste(gsub('M','M-',rev(row.names(id_est))[c(1,2,4)]),collapse=',')}
(Figs. \ref{fig:w_erosion} \& \ref{fig:ids_comp}). A fourth, M-209,
...
grain rice cultivars developed by the California Cooperative Rice
Research Foundation (California, USA), for the last 6 years
...
There is estimated to be an decrease of yield over time once a
cultivar is selected (i.e., yield erosion) (\Sexpr{yrs_coef} kg
ha\tsups{-1} year\tsups{-1}, 90\% CI: \Sexpr{RES_CI[2]} to
\Sexpr{RES_CI[1]}). The oldest cultivar in the trials is estimated to
have lost an average of \Sexpr{round(yrs_coef*-36/1000,1)} Mg
ha\tsups{-1} since release. Comparing two dominate medium grain rice
cultivars M-202 (widely planted from the late 1980s until the early
2000s), and M-206 (the most widely planted CCRRFS cultivar at
approximately 50\% of planted area at the time of writing), the two
cultivars experienced similar trends over time to each other
(Fig. \ref{fig:compare_yrs}) and to the estimate from all released
medium grains (Table \ref{tbl:coef}). At the time of writing, an
estimated \Sexpr{m202_v_206} kg ha\tsups{-1} yield advantage of M-206
over M-202 can be attributed solely to the erosion of M-202's yield
over time (Figs \ref{fig:w_erosion} \& \ref{fig:compare_yrs}). Further
supporting this estimate, both cultivars yielded similarly at their
...
We estimate that yield erosion has decreased the yield of medium
grain rice cultivars released by the CCRRF over the last 30+ years by
an average of \Sexpr{yrs_coef} Mg ha\tsups{-1},
potentially offsetting any gains made by the breeding program. Biotic
and abiotic forces have previously been speculated to be responsible
for yield erosion, since the cultivar's genetics remain stable from the
time of selection but the conditions the cultivar may experience over
time are continuously changing \citep{Peng1998, Peng2000,
Peng2010}. The trend observed here is consistent with previous reports
of the erosion of genetic gains of cultivar IR8 at IRRI
\citep{Peng1998, Peng2000}, suggesting the erosion of yield over time
is a phenomenon that widely affects rice breeding programs. This
highlights both the need for maintenance breeding and the difficulty
of increasing genetic yield potential, especially in systems such as
CA where yield is approaching the physiological limit
\citep{espe16a}. Without maintenance breeding continuously selecting
cultivars well adapted to current conditions, CA rice production
systems are predicted to have lost an estimated
\Sexpr{round(yrs_coef*(-36/1000),1)} Mg ha\tsups{-1} grain yield over
the study period.

Overall, we do not find conclusive evidence of increased genetic yield
...
\Sexpr{max(mod_data[,'year'])}. After correcting yield comparisons for
yield erosion over time, the estimated yield improvement spans from
\Sexpr{rel_CI[1]} to \Sexpr{rel_CI[2]} kg ha\tsups{-1}
year\tsups{-1}. This suggests that both increases and decreases in the
genetic yield potential over the study period. However, even when the
comparison is not corrected for yield erosion, the estimated
improvements over time (\Sexpr{only_release_coef} kg ha\tsups{-1}
year\tsups{-1}; Fig. \ref{fig:wo_erosion}) are still much less than
previous estimates of 37 to 60 kg ha\tsups{-1} year\tsups{-1}
\citep{Omar2014}. We conclude that the breeding program has likely
been maintaining but not substantially improving yield potential in
the face of changing biotic and abiotic pressures.
...
ha\tsups{-1} in side-by-side comparison on top of the 0.5 Mg
ha\tsups{-1} target improvement. Often realized side-by-side yield
improvements are closer to 2 to 3\% per release, in which case no
absolute improvements are made beyond maintenance. The evidence here
is consistent with the latter situation occurring in CA, further
suggesting that the genetic yield potential has not increased
substantially in the last 30+ years (Fig. \ref{fig:w_erosion}).
...
even as little to no gains have been seen in the breeding trials
...
There are several cultivars that were estimated to have better than
...
grain yields \citep{espe2017}, and it is also known that the RES
...
period and are no less important. However, planning to meet future
