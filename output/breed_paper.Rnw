...
potential. This hypothesis was tested using 33 years of experimental
yield data from the California Cooperative Rice Research Foundation
Rice Experiment Station. Based on side-by-side comparisons of old and
new rice varieties which do not consider yield erosion, there was an
apparent increase in yield. However, the yields of older varieties
eroded at an estimated rate of 29.3 kg ha\tsups{-1} year\tsups{-1}
(90\% credible interval -4.4 to -53.3) after initial selection. Once
this effect was considered, the yield advantage of newer cultivars
over old was uncertain (-3.3 kg ha\tsups{-1} year\tsups{-1}, 90\%
credible interval -36.1 to 31.5). These results highlight first, the
importance of continuous crop improvement and deployment of new
cultivars simply to maintain existing yields, and second, to increase
the genetic yield potential, higher yield targets are
...
the model was used as described above except omitting the
$Rate_{yield\ erosion\ i}$ term (Model 2). Lastly, a model was fit to estimate
if there were differences between cultivars in the rate of yield
erosion via a random slope by cultivar (Model 3).
...
also conducted using classical methods, specifically a frequentist mixed-effects
...
and classical approaches for all models, with the estimates being
similar between approaches for similar models. The Bayesian estimates
...
(Fig \ref{fig:first_yrs}. Evaluating these initial yields over time
provides evidence of genetic yield improvements during the 33-year
study period. Individual exceptions to this were two cultivars (M-204
and M-205) that had higher yields than the others.
...
the rate of yield erosion (Model 3), hence the model excluding these
effects (Model 2) is presented here. The yields of each cultivar over
time were evaluated relative to when the cultivar was released using
the full model which accounts for both yield erosion and yield
improvements over time. The average decrease in yield over time (i.e.,
yield erosion) once a cultivar was selected was estimated at
\Sexpr{yrs_coef} kg ha\tsups{-1} year\tsups{-1} (90\% CI:
\Sexpr{RES_CI[2]} to \Sexpr{RES_CI[1]}) (Fig \ref{fig:RES_year}; Table
\ref{tbl:coef}). The oldest cultivar in the trials
(\Sexpr{gsub("M","M-",unique(mod_data[which.min(mod_data[,"release_yr"]),"id"]))})
is estimated to have lost an average of \Sexpr{round(yrs_coef*-36/1000,1)} Mg
...
2000s) experienced similar trends in yield decline. M-206 replaced M-202
as the most widely grown cultivar in CA and the current estimated 560
kg ha\tsups{-1} yield advantage of M-206 over M-202 can be attributed
solely to the erosion of M-202's yield over time (Figs
\ref{fig:first_yrs} \& \ref{fig:compare_yrs}). Further supporting this
estimate of yield erosion, both cultivars yielded similarly at their
...
\caption{The performance of two popular cultivars over time. The
  dashed line is the estimated yield erosion as estimated by the full
  model. Points ($\bullet$) represent the mean, and bars (|) the standard
  error of the observed data.}
\label{fig:compare_yrs}
\end{figure}

When yield erosion is not accounted for in the analysis (i.e.,
omitted from the model), there is estimated to be an increase per year
of approximately \Sexpr{only_release_coef} kg ha\tsups{-1}
year\tsups{-1} (90\% CI \Sexpr{only_release_CI[1]} to
\Sexpr{only_release_CI[2]}) (Fig. \ref{fig:wo_erosion}). However, the
full model which accounts for both the year of release and yield
erosion indicates that the estimated change in grain yield per year
in the CCRRF's breeding trials at the RES was \Sexpr{release_yr_coef}
kg ha\tsups{-1} year\tsups{-1}, with both small positive and negative
trends within the credible interval (90\% CI: \Sexpr{rel_CI[1]} to
\Sexpr{rel_CI[2]}) (Fig. \ref{fig:first_yrs}).
...
the 33 year period of this study are M-202, M-206, and M-105. Cold
...
with over 70\% of the area in these cultivars at any given time during the study
...
and M-402 which is a premium quality cultivar. Other cultivars such as
...
Not accounting for yield erosion, the estimated yield improvement of
\Sexpr{only_release_coef} kg ha\tsups{-1} year\tsups{-1}
(Fig. \ref{fig:wo_erosion}) that does not account for yield erosion is
lower than the 37 to 60 kg ha\tsups{-1} year\tsups{-1} estimated
by~\citet{Omar2014} in side-by-side comparisons. However, when
accounting for yield erosion and looking acrossall cultivars, there is
no conclusive evidence of increased genetic yield potential in medium
...
plausible. This analysis suggests that the CCRRF breeding
...
M-205 did have higher yield potential; however, this cultivar is not
...
may influence yields with either warmer~\citep{van2013} or cooler
 temperatures reducing yields. Again, this is
unlikely to be the case in this situation as ~\citet{espe2017} found
no significant changes in growing season temperatures (either high,
low or average temps) the Sacramento Valley during the study period.
Thirdly, biotic and abiotic forces have previously been speculated as
the cause for yield erosion of the cultivar IR8 at
IRRI~\citep{Peng1998, Peng2000, Peng2010}. They argued that since the
cultivar's genetics remain stable from the time of selection but the
conditions the cultivar experiences over time are continuously
changing leading ot yield erosion~\citep{Peng1998, Peng2000,
Peng2010}. However, it is not clear what is responsible for the yield
erosion of \Sexpr{yrs_coef} Mg ha\tsups{-1} year\tsups{-1} found in
this analysis. It is unlikely biotic factors as pest and disease
pressure are low in this Mediterranean climate. Furthermore, these
fields are fallowed every other year and the residues are burned,
further lowering the potential for pest and disease pressure. While
efforts should be made to identify the cause of erosion in this
environment, this observation highlights both the need for maintenance
of plant breeding and the difficulty of increasing genetic yield
~\citep{espe2017} potential, especially in systems such as CA where yield is approaching
the physiological limit \citep{espe16a}. Without maintenance breeding
(i.e., continuously selecting cultivars well adapted to current
conditions), CA rice production systems are predicted to have lost an
estimated \Sexpr{round(yrs_coef*(-36/1000),1)} Mg ha\tsups{-1} grain
yield over the study period.
...
\citep{espe2017}. Hence, cultivars which perform well at the RES might
not perform well across of CA. Since these stressful temperature
events are inconsistent across years, they influence yield stability (how consistent
yield is year to year). Therefore, it should be noted that while our
...
period and are important. Maintaining high quality is imperative for
the breeding program and it is not uncommon in the breeding program
...
consideration. Another change in CA rice cultivars is that crop
duration has been reduced. For example, during this study period,
M-202 and M-206 have been the predominate cultivars grown in CA, and
while their yield potentials were similar at the time of release
(Fig. 5), M-206 matures about 6 days earlier than M-202 (Table
...
\ref{fig:compare_yrs}). While M-205 has increase yield potential, it
is not broadly adapted. Its adoption (although limited) in the
...
these state wide yield gains are far less than those experienced
during the preceding era, and by their nature agronomic gains cannot
surpass the genetic yield potential \citep{Fischer2015}. Judging from
yields achieved at the RES (Figs. \ref{fig:RES_year} \&
\ref{fig:compare_yrs}), a researcher-managed production system
implementing best management practices in more controlled situations
compared to farmers' fields, state average yields (approximately 10 Mg
ha\tsups{-1} at the time of writing; Fig. \ref{fig:state_yield}) may
be approaching the current genetic limit. Assuming that the level of
management possible in small experimental plots cannot be attained in
farmers' fields and hence experimental yields represent the attainable
yield ceiling \citep{Fischer2015}, agronomic gains in farmer's fields
are nearing their potential, consistent with physiological model-based
estimates of the attainable yield gap in CA rice production systems
\citep{espe16a}.
