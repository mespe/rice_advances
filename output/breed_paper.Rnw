...
Meeting the challenge of feeding an growing population with
...
improvement in recent years despite demonstrated increased yield
potential of new genetics over old in side-by-side comparisons. We
analyzed experimental data from one such system, California (CA) rice
production, in order to better understand what improvements have been
made in the genetic yield potential via plant breeding. Specifically,
the hypothesis that a rice variety's yield erodes over time after the
initial selection was tested as an explanation for how side-by-side
comparisons show increased yield while no substantive increases have
been seen in farmers' fields. We confirmed an apparent increase in
yield in side-by-side comparisons between old and newer varieties when
the difference between the years since release was not
considered. However, we demonstrate this apparent increase in yield
can be explained by the differences in the time since selection
between cultivars. The impact of yield erosion was estimated to be
29.8 kg ha\tsups{-1} year\tsups{-1} after initial
selection. Once this effect was considered, the apparent yield
advantage of newer cultivars over old was uncertain. This underscores
the importance of breeding to maintain yield and the challenges of
increasing the genetic yield potential.

\end{abstract}

\begin{keyword} rice, yield potential, breeding
\end{keyword}
...
surpassing supply via continued yield improvement of staple crops,
however more and more production systems are experiencing plateaus in
grain yield \citep{Grassini2013, Peng2010}. If continued improvements
per unit land area are no longer possible, increasing the area under
...
importance to better understand why grain yields have slowed or
plateaued in intensified production systems.
...
more slowly (Fig. \ref{fig:state_yield}) despite continuous plant
improvement \citep{ccrrf}. For the last 30 years, each newly developed
...
with ``check'' cultivars). Yet, taken across years, these same reports
show stagnant yields. For example, at the year of release (1985),
cultivar M-202 averaged 11 Mg ha\tsups{-1} \citep[14\%
MC;][]{m202}. In 2015, the reported average yield of the newest released
cultivar M-209 was 10.8 Mg ha\tsups{-1} \citep{m209}. Yet this same
report, M-209 demonstrated a X\% yield advantage over M-202. In other rice
systems, a decrease in a cultivar's yield performance over time has
been observed \citep{Peng1998,
Peng2010, Peng2000, Datta1995}, often termed yield erosion
\citep{Peng1998}. It has been speculated that this is due to the
inability of a selected cultivar's genetics to adapt to changing  biotic and
abiotic conditions. We sought to quantify this phenomenon to better
understand how plant improvement and yield erosion are contributing to
the slow yield improvement in farmers' fields over the last
30 years in CA. Specifically, two competing hypotheses were
tested: (1) the yield advantage of each newly developed cultivars is
greater than previous cultivars, and (2) the erosion of yield over
time is creating the illusion of improved yields (i.e., the new
cultivar is recovering the old benchmark as the older cultivar's yield
erodes). 
...
\caption{Average rice yields over the period \Sexpr{min(CA_data$Year)}
  -- \Sexpr{max(CA_data$Year)} for California, USA, as reported by the
...
top 15cm. As the central location for the CCRRF breeding program, the RES is
...
Plots at the RES are managed similar to prescribed best management
...
variety was officially released and the year of release using a
mixed-effects linear regression. To account for similarities between years
and between cultivars, year and cultivar were included in the model as
random effects.
...
release or the year of release using the `loo' package \citep{loo},
an efficient means of conducting leave-one-out cross validation
\citep{loo_paper}, with the full model estimated to have slightly
better predictive accuracy compared to the reduced models.

\section{Results}\label{results}

\subsection{Genetic yield improvements}

There was no conclusive evidence that the yields of rice cultivars
selected by CCRRF have, on average, increased over the last 30
years. The estimated change in grain yield per year was estimated to
be a negligible \Sexpr{release_yr_coef} kg ha\tsups{-1}
year\tsups{-1}, with both small positive and negative trends within
the credible interval (90\% CI: \Sexpr{rel_CI[1]} --
\Sexpr{rel_CI[2]}). Hence, the evidence here is consistent with little
to no increase in yield potential over the study period. When yield
erosion is not corrected for in the comparison (i.e., omitted from the
model), there is an apparent small increase
per year (approximately \Sexpr{only_release_coef} kg ha\tsups{-1}
year\tsups{-1}; 90\% CI \Sexpr{only_release_CI[1]} --
\Sexpr{only_release_CI[2]}) (Fig. \ref{fig:release}), though this
estimate ignores the confounding effect of yield erosion over
time.

There do appear to be some differences between cultivars, with some
cultivars showing higher yield than the overall average. The marginal
estimate for the performance of a cultivar (averaged over the
uncertainty in the other parameters in the model) shows three
cultivars with higher than average yields:
\Sexpr{paste(gsub('M','M-',rev(row.names(id_est))[c(1,2,4)]),collapse=', ')}
(Fig. \ref{fig:ids_comp}). A fourth, M-209, might also have higher
than average yield, though the CI overlaps with zero. 
...
@
\caption{The average yield ($\bullet$) and standard error (|) of
medium grain rice cultivars by year of release (a.) without correcting
for yield erosion over time and (b.) accounting for yield erosion over
time. The trend without correcting for yield erosion (a.) was estimated by
omitting the year of each cultivar's release from the
model. Yield erosion was corrected for by including the years since
release (b.) in the model. predictions assuming no yield erosion
were generated from the model which included both the year of release
and the years since release. For display purposes, only data for
experimental plots planted at the California Cooperative Rice Research
Foundation Rice Experiment Station (California, USA) over the years
\Sexpr{min(comp_yrs)} -- \Sexpr{max(comp_yrs)} are included.}
\label{fig:release}
\end{figure}
...
\caption{The relationship between yield and the years since release
  ...
  (RES), California, USA. The estimated erosion of yield over time
  (averaged over differences in cultivar and year) is
  estimated to be \Sexpr{yrs_coef} kg ha\tsups{-1}
  year\tsups{-1}. Points ($\bullet$) and bars (|) represent the mean and standard
  error of the raw data, respectively. The
  shaded area is the 90\% credible interval.}
\label{fig:RES_year}
\end{figure}

There is estimated to be a small loss of yield over time once a
cultivar is selected (i.e., yield erosion) (\Sexpr{yrs_coef} kg
ha\tsups{-1} year\tsups{-1}, 90\% CI:
\Sexpr{RES_CI[2]} -- \Sexpr{RES_CI[1]}). The oldest cultivar in the
trials is estimated to have lost \Sexpr{round(yrs_coef*-36/1000,1)} Mg
ha\tsups{-1} since release. Comparing two dominate
medium grain rice cultivars (M-202, which was widely planted from the late 1980s until
the early 2000s, and M-206, the most widely planted CRRFS
cultivar at approximately 50\% of planted area at the time of
writing), the two cultivars experienced similar trends over time to
each other (Fig. \ref{fig:compare_yrs}) and to the estimate from all
released medium grains (Table \ref{tbl:coef}). There is an estimated
\Sexpr{m202_v_206} kg ha\tsups{-1} yield advantage of M-206
over M-202 at the time of writing due solely to the erosion of M-202's
yield over time (Figs \ref{fig:release} \&
\ref{fig:compare_vars}). Further supporting this estimate, both cultivars yielded
similarly at their respective year of release
(Fig. \ref{fig:compare_yrs}), despite the fact that M-202 was released
19 years prior to M-206 (Fig. \ref{fig:release}, Table \ref{tbl:ids}).
...
\caption{The performance of two major cultivars at the CA Rice
  Experiment Station (RES) over the period
  \Sexpr{min(mod_data[,'year'])} -- \Sexpr{max(mod_data[,'year'])}. The
  solid line represents a simple linear regression of yield for each
  cultivar over the period which it was planted at the RES (fit to only
  that cultivar's data), while the dashed line is the published 3-year
  average yield of M-202 at its release (1982-1984). Both cultivars have
  a similar yield at their respective year of release, and similar
  decreases over time. The slope for the individual regressions are
  comparable to the slope estimated from all medium grain cultivars
  (\Sexpr{yrs_coef} kg ha\tsups{-1} year\tsups{-1}).}
\label{fig:compare_yrs}
\end{figure}

\section{Discussion}

Overall, we do not find conclusive evidence of increased yield
potential increasing in medium grain rice cultivars released by
CCRRF's breeding improvement program over the period
\Sexpr{min(mod_data[,'year'])} to \Sexpr{max(mod_data[,'year'])}. After correcting yield comparisons
for yield erosion over time in the older cultivar, the estimated yield
improvement spans from \Sexpr{rel_CI[1]} -- \Sexpr{rel_CI[2]} kg
ha\tsups{-1} year\tsups{-1}.  Even when the
comparison is not corrected for differences in years since release
between cultivars, the estimated improvements over time
(\Sexpr{only_release_coef} kg ha\tsups{-1}
year\tsups{-1}; Fig. \ref{fig:release} dashed line) are
still much less than previous estimates of 37 -- 60 kg
ha\tsups{-1} year\tsups{-1}
\citep{Omar2014}. Given this evidence, we conclude that the breeding
program has likely been maintaining but not substantially improving
yield potential in the face of changing biotic and abiotic
pressures. These biotic and abiotic forces are expected to erode yield
over time \citep{Peng1998, Peng2000, Peng2010}. In fact, the trend
seen here is extremely consistent with previous reports of erosion of
genetic gains of cultivar IR8 at IRRI \citep{Peng1998, Peng2000}. The
evidence here further supports that the erosion of yield over time is
a phenomenon that affects all rice breeding programs. This further
highlights both the need for maintenance breeding and the difficulty
of increasing genetic yield potential as yield approaches the
physiological limit. Without maintenance breeding continuously
selecting cultivars well adapted to current conditions, CA rice
production systems are predicted to have lost an estimated
\Sexpr{round(yrs_coef * (-36/1000), 1)} Mg ha\tsups{-1}
grain yield over the study period.

The goal of a breeding program is to not only maintain the
benchmark yield of the previous ``standard'' but to surpass it. Given
the estimated yield decreases over time, to hit an ambitious target of
0.5 Mg ha\tsups{-1} (approximately 5\%) increase in yield
per release (with roughly 4 -- 5 years between releases), a new cultivar
needs to yield an additional \Sexpr{round(yrs_coef*-4,-1)} --
\Sexpr{round(yrs_coef*-5,-1)} kg ha\tsups{-1} in
side-by-side comparison on top of the 0.5 Mg ha\tsups{-1}
target improvement. Often realized side-by-side yield
improvements 
are closer to 2 -- 3\% per release, in which case no absolute
improvements are made beyond maintenance. The evidence here is
consistent with the latter situation occurring in CA, further suggesting
that the genetic yield potential has not increased substantially in
the last 30+ years (Fig. \ref{fig:release}). 
...
@ 
\caption{A conceptual diagram highlighting three scenarios:
  (1) maintenance breeding, where losses over time are offset by
  improvements via plant improvement, (2) no maintenance breeding,
  where grain yield slowly erodes over time, and (3) absolute
  improvement in grain yield, where gains in grain yield exceed the
  losses over time.}
\label{fig:theory}
\end{figure}

That is not to say there have not been yield improvements over the
last 30 years. California state average yields have increased slowly,
even as genetic gains have been small
(Fig. \ref{fig:state_yield}). The evidence here suggests mechanisms
other than increased genetic yield potential are responsible
...
Pittelkow2012, linquist2008}, etc. may all be contributing to
increases in the state average yield. However, these gains are less
than those experienced during the ``Green revolution'', and by their
...
practices in more controlled situations compared to farmers, state
average yields (approximately 10 Mg ha\tsups{-1} at the time
of writing; Fig. \ref{fig:state_yield}) may be approaching the current
...
improvement, including increased quality \citep{mckenzie1993} and milling yield
\citep{mckenzie1994} have been made over the study period and are no
less important. However, planning to meet future demand necessitates
increases in grain yield \citep{Godfray2010, Foley2011a, Mueller2012}.
Meeting the challenge to produce more grain without expansion of land
under cultivation will rely on new genetics with increased yield
...
decreases in the current ``standard'' cultivars' yield over
time (Fig. \ref{fig:theory}). While side-by-side comparisons will continue to be useful,
comparisons across years to each cultivar's benchmark yield at the
time of release should provide information on the absolute
improvements being made in yield potential over time. New technologies
may need to be adapted to further increase yield potential, including
hybrid rice.
...
improvements, we show here that (1) continuous maintenance via plant
...
rownames(RES_coef) = c("Intercept (kg ha\\tsups{-1})",
                       "Release year (kg ha\\tsups{-1} year\\tsups{-1})",
                       "Years since release (kg ha\\tsups{-1} year\\tsups{-1})")
...
colnames(ids) = c("Cultivar", "Release Year")
ids = ids[order(ids[,2]),]
