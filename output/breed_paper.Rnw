...
Research Foundation (CCRRF) is located northwest of Biggs, CA (39.4648,
-121.7342), and has been the central location for the development of
improved rice varieties suited for CA since 1969. The climate at the
RES is Mediterranian, characterized by mild winters during which most
of the annual precipitation occurs (\Sexpr{18*24.5} -- \Sexpr{25*24.5}
mm annually) and warm summers largely free of precipitation
events. Soils at the RES are classified as Esquon-Neerdobe clays with
roughly 2\% SOM \citep{ssurgo} in the top 15cm. As the central
location for the breeding program, the RES is researcher-managed (as
opposed to farmer-managed trials) and conducts the bulk of the trials
for the Statewide Variety Trials (approximately 30\% of the total
plots are planted at RES). For these reasons, we focused our
investigation on the RES to avoid complications due to site, climate, and
management differences at the other 6 -- 8 farmer-managed trials.
...
90\% of the planted area in CA. This lead to a final data set of
\Sexpr{format(nrow(mod_data), big.mark = ',')} observations with
\Sexpr{length(unique(mod_data[,'id']))} cultivars represented,
constituting every publicly released medium grain variety developed by
...
\begin{equation}
 yield_i = \alpha + \beta_{years\ since\ release\ i}* x_{years\ since\
   release\ i} + \beta_{year\ i} + \beta_{cultivar\ i}
\end{equation}
...
release), $\beta_{year}$ is the random effect of year, and
$\beta_{cultivar}$ is the random effect of cultivar. A similar but
separate model was used to estimate the yield advantage of newer
releases over older ones:

\begin{equation}
yield_i = \alpha + \beta_{release\ year\ i}*x_{release\ year\ i} +
\beta_{year\ i} + \beta_{cultivar\ i}
\end{equation}


Where $\beta_{release\ year\ i}$ is effect of $x_{release\ year\ i}$,
the year of release for $i$'s cultivar. All other parameters are
similar to Eq. 1.
...
<<echo=FALSE>>=
rel_CI = round(posterior_interval(RES_rel_fit, pars = "release_yr_c") * scl, 1)

posterior_interval(RES_rel_fit, pars = "release_yr_c") * scl
m202_v_206 = ((posterior_predict(RES_rel_fit, newdata = data.frame(release_yr_c = c(-15, 4)),
...
yield, with an estimated \Sexpr{round(fixef(RES_rel_fit)[2] * scl, 1)}
...
\Sexpr{m202_v_206[2]} kg ha\textsuperscript{-1} yield advantage of M-206
over M-202 (Figs \ref{fig:release} \& \ref{fig:compare_yrs}).

\begin{figure}[ht]
  
<<fig=TRUE, echo=FALSE>>=
yrs <- 1980:2015
pp <- posterior_predict(RES_rel_fit, re.form = NA, newdata = data.frame(release_yr_c = yrs - 2000))
pp <-  (pp * scl + center)

ints <- apply(pp, 2, quantile, p = c(0.05, 0.5, 0.95))

res = mod_data$site == "RES"
plot_data = prep_plot_data(mod_data$yield_kg[res] ,
                           mod_data$id[res])
...
plot(yield_kg ~ release_yr, data = plot_data, pch = 16, #col = rgb(0,0,0,0.25),
     ...
     ylim = c(8000, 12500),
     xlim = c(1980,2019))
abline(a = (((fixef(RES_rel_fit)[1] * scl) + center) ) -
           (2000 * fixef(RES_rel_fit)[2] * scl),
       b = fixef(RES_rel_fit)[2] * scl, col = "grey50",
       lty = 2)
...
text(1990, 8000, bquote(.(round(fixef(RES_rel_fit)[2] * scl , 1))~kg~ha^-1~year^-1))
...
plot_data = lapply(tars, function(x) {
    ...
    tmp = prep_plot_data(dd$yield_kg , dd$year)
...
invisible(sapply(1:2, function(i){
    plot(mean ~ year,
         data = plot_data[[i]],
         xlim = c(1995, 2016), ylim = c(7500,13500),
...
\caption{The performance of two major cultivars at the CA Rice
  Experiment Station (RES) over the period 1995 -- 2015. The line is the average
  yield for each cultivar over the period which it was planted at the
  RES. The newer cultivar, M-206, appears to have a slight yield
  advantage (an estimated \Sexpr{m202_v_206[2]} kg ha\textsuperscript{-1})over the older cultivar, M-202.}
...
<<echo=FALSE>>=
all_CI <- round(posterior_interval(RES_yrs_fit, pars = "yrs_in_trial") * scl, 1)
RES_CI <- round(posterior_interval(RES_yrs_fit, pars = "yrs_in_trial") * scl, 1)
@ 
...
\Sexpr{round(fixef(RES_yrs_fit)[2] * scl, 1)} kg
...
\Sexpr{round(fixef(RES_yrs_fit)[2] * scl * 36 , 1)} kg
...
with(plot_data, segments(x0 = yrs_in_trial, y0=yield_kg + yield_se,
                         y1 = yield_kg - yield_se))
abline(a = ((fixef(RES_yrs_fit)[1] * scl) + center) ,
       b = fixef(RES_yrs_fit)[2] * scl, col = "grey50",
       lty = 2)
ints <- apply(pp, 2, quantile, p = c(0.05, 0.5, 0.95))
text(5, 8000, bquote(.(round(fixef(RES_yrs_fit)[2] * scl , 1))~kg~ha^-1~year^-1))

@
\caption{The relationship between yield and the years since release for
  ...
  using only these data (\Sexpr{round(fixef(RES_yrs_fit)[2] * scl, 1)}
...
plot_data = lapply(tars, function(x) {
    dd = mod_data[mod_data$id == x & mod_data$site == "RES",]
    tmp = prep_plot_data(dd$yield_kg , dd$yrs_in_trial)
...
segments(x0 = plot_data[[2]]$yrs_in_trial,
         ...
         col = "grey40")
abline(lm(yield_kg ~ yrs_in_trial,
