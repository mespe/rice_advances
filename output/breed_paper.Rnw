...
quantify improvements made in the genetic yield potential obtained
through plant breeding. California rice systems are among the highest
in the world and close to maximum yield potential. Specifically, the
hypothesis was tested that if rice cultivar yields decline over time
(termed yield erosion) then apparent yield increases in side-by-side
yield comparison tests will not reflect increases in yield
potential. This hypothesis was tested using experimental yield data
over 30+ years at the California Cooperative Rice Research Foundation
Rice Experiment Station. Results show an apparent increase in yield in
side-by-side comparisons between old and new rice varieties when the
yield erosion was not considered. However, the yields of older
varieties eroded at an estimated rate of 29.3 kg ha\tsups{-1}
...
year\tsups{-1}, 90\% credible interval -36.1 to 31.5). This suggests
that side-by-side comparisons reflected the decreasing yields of older
cultivars more than the increasing yield of newer ones. These results
highlight the importance of continuous crop improvement and deployment
of new cultivars as yield will decline over time without it. To
increase the genetic yield potential of rice in the face of this
phenomenon, higher yield targets are needed to counter-act the
decrease over time in the current cultivars.
...
grain yield~\citep{Grassini2013, Peng2010}. If this trend continues,
...
and relatively free of pests and diseases. These conditions lead to
some of the highest yields in the world~\citep{FAO2016}. Production in
CA is predominately focused on premium quality medium grain
...
slow and \citet{espe16a} reported that indeed this was the case
...
varieties have increased yield potential''?  Like many breeding
programs, one of the goals of the CCRRF is to increase yields through
breeding varieties with increased yield potential. However, when the
...
MC;][]{m209}. However, in annual breeding reports and varietal release
announcements, data based on yield performance in side-by-side variety
trails show that new varieties usually out yield the older
varieties. In other rice systems, a decrease in a cultivar's yield
performance over time has been observed~\citep{Peng1998, Peng2010,
Peng2000, Datta1995}, often termed yield erosion~\citep{Peng1998}. It
has been speculated that this yield erosion is due to the inability of
...
side-by-side yield comparisons show yield improvements in varieties
...
yield comparison tests will not reflect true or absolute yield
increases over time.
...
Agriculture National Agricultural Statistics Service, 2017). Rapid
gains in grain yield were experienced from approximately 1950 -- 1990,
but annual gains since 1990 have been slower.}
...
1969. The California Statewide Variety Trails evaluate current and
promising varieties at the RES and six to eight on-farm trials around
the state each year. To avoid potential complications due to site,
climate, and management differences at the farmer-managed trials per
year, we focused our investigation on the RES. The RES is
...
Esquon-Neerdobe clays with roughly 2\% soil organic matter in the top 15cm~\citep{ssurgo}.
...
90\% of the planted rice area in CA. Newly developed entries are typically
evaluated in these trials for 3 to 5 years prior to
their official release. For these cultivars planted in breeding trials
at the RES from \Sexpr{min(mod_data[,'year'])} --
\Sexpr{max(mod_data[,'year'])} plot level observations (replicated
3 to 4 times in a CRB design) were collected leading to a final data set
...
year of public release and the time elapsed (in years) since release was
determined.

Experimental plots at the RES are managed similar to University of California
Cooperative Extension prescribed best management
practice for land preparation, fertility rates, and pest
and disease control~\citep{ucce}. As is typical for water-seeded
systems, the plots were planted by pre-germinating rice seed prior
...
MC prior to reporting.

\subsection{Statistical analysis}

To test the hypothesis, the influence of breeding and yield erosion
were quantified on yield over time using a Bayesian hierarchical
model. Yield was modeled as a response to the number of years since
...
average yield for the RES across cultivars and years, $Rate_{yield\
...
interval~\citep{rstanarm}. To verify the estimates, the analysis was
also conducted using classical methods (frequentist mixed-effects
model fit by maximum likelihood; See supplemental material for
details). Complete data and code used for this
...
There was general agreement between the estimates from the Bayesian and
classical approaches.  The Bayesian estimates are presented here due
to the inherent flexibility of Bayesian inference, 
though the classical estimates can be found in the supplemental
material. Comparing the cultivars' yield during each's initial time
(average 3 years) in the experimental trials just before being
released showed that across cultivars, yields ranged from
\Sexpr{round(plot_data["M207", "mean"], 2)} (M-207) to
\Sexpr{round(plot_data["M205", "mean"], 2)} Mg ha\tsups{-1}
(M-205). Evaluating these initial yields over time indicates there was
no evidence of genetic yield improvements during the 30-year study
period (Fig. 2). Individual exceptions to this were two varieties
(M-204 and M-205) that had higher yields than the others.
...
The yields of each cultivar were evaluated over time relative to the
time when the cultivar was released using the full model which
accounts for both yield erosion and yield improvements over time. The
average decrease in yield over time (i.e., yield erosion) once a
cultivar was selected was estimated \Sexpr{yrs_coef} kg ha\tsups{-1}
year\tsups{-1} (90\% CI: \Sexpr{RES_CI[2]} to \Sexpr{RES_CI[1]}) (Fig
\ref{fig:RES_year}; Table \ref{tbl:coef}). The oldest cultivar in the
trials (\Sexpr{gsub("M","M-",unique(mod_data[which.min(mod_data[,"release_yr"]),"id"]))})
had lost an average of \Sexpr{round(yrs_coef*-36/1000,1)} Mg
ha\tsups{-1} since its release. The most dominate medium grain rice
cultivar, M-202 (widely planted from the late 1980s until the early
2000s) experienced similar trends in yield decline
(Fig. \ref{fig:RES_year}; Table \ref{tbl:coef}). M-206 replaced M-202
as the most widely grown cultivar in CA and the current estimated 560
kg ha\tsups{-1} yield advantage of M-206 over M-202 can be attributed solely
to the erosion of M-202's yield over time (Figs \ref{fig:first_yrs} \&
...
When yield erosion is not corrected for in the analysis (i.e.,
omitted from the model), there is an apparent small increase per year
(approximately \Sexpr{only_release_coef} kg ha\tsups{-1}
year\tsups{-1}; 90\% CI \Sexpr{only_release_CI[1]} to
\Sexpr{only_release_CI[2]}) (Fig. \ref{fig:wo_erosion}).  However,
using the full model which corrected for both the year of release and
yield erosion, indicates that the estimated change in grain yield per
...
After correcting for yield erosion, some cultivars were higher
yielding compared to the overall average (as estimated by random
intercepts). The marginal estimate for the performance of a cultivar
averaged over the uncertainty in the other parameters in the model
(i.e., the random effect) shows three cultivars with higher than
average yields:
...
In California, the cultivars with broad adaptation and adoption over
the 30+ year period of this study are M-202, M-206, and M-105. Cold
...
ocean winds through the Delta ~\citep{espe16a}. These three
cultivars have a higher degree of cold tolerance, high yield
potential, and relatively short duration and are thus broadly grown
with over 70\% of the area in one of these cultivars during the study
period~\citep{ucce}. Importantly, not all medium grain cultivars that were
released were purported to have higher yields than previous
cultivars. Some cultivars had special traits, but upon release had
similar yields to more broadly adapted cultivars. Examples of this
include M-208, which had resistance to blast disease (\textit{Magnaporthe grisea})
and M-402 which is a premium quality variety. Other varieties such as
M-205 and M-209, while high yielding, do not have the cold tolerance
of the other cultivars and are thus only recommended for the northern
portion of the Sacramento Valley.

Looking at all the cultivars that have been released, there is no
conclusive evidence of increased genetic yield potential in medium
grain rice cultivars released by CCRRF's breeding improvement program
over the period \Sexpr{min(mod_data[,'year'])} to
\Sexpr{max(mod_data[,'year'])}, which has stayed relatively constant
at \Sexpr{round(mean(mod_data[,"yield_kg"])/1000, 1)} Mg
ha\tsups{-1}. Since the estimated yield improvement spans from
\Sexpr{rel_CI[1]} to \Sexpr{rel_CI[2]} kg ha\tsups{-1} year\tsups{-1}
(Fig. \ref{fig:RES_year}; Table \ref{tbl:coef}),
both increases and decreases in the genetic yield potential over the
study period are plausible. The estimated yield
improvement of \Sexpr{only_release_coef} kg ha\tsups{-1}
year\tsups{-1} (Fig. \ref{fig:wo_erosion}) that does not account for
yield erosion is lower than the 37 to 60 kg ha\tsups{-1}
year\tsups{-1} estimated by~\citet{Omar2014} in side-by-side
comparisons. Rather, this analysis suggests that the CCRRF breeding
program has maintained but not substantially improved the yield
...
and M-105). Yields of these varieties at time of release have all
yielded between
...
and
\Sexpr{round(min(by(tmp,tmp[,"id"],function(x)mean(x[,"yield_kg"]))/1000),2)}
Mg ha\tsups{-1} with no indication of yields increasing
(Fig. \ref{fig:first_yrs}). While these yields have not increased,
M-205 did have higher yield potential; however, this variety is not
broadly adapted due to poor cold tolerance.
...
here as the fields used in this study grow rice only once season a
year and tend to be non-flooded for the remainder of the
year. Furthermore, at this location and unlike most CA rice fields,
the fields are left fallow every other year. Second, changing climate
may influence yields with either warmer~\citep{van2013} or
cooler (Espe et al., 2017) temperatures reducing yields. Again, this
is unlikely to be the case in this situation as Espe et al. (2017)
found no significant changes in growing season temperatures (either
high, low or average temps) the Sacramento Valley during the study
period.  Thirdly, yield erosion of a cultivar as has been reported for
the cultivar IR8 at IRRI~\citep{Peng1998, Peng2000, Peng2010}. Biotic
and abiotic forces have previously been speculated to be responsible
for yield erosion, since the cultivar's genetics remain stable from
the time of selection but the conditions the cultivar may experience
over time are continuously changing~\citep{Peng1998, Peng2000,
Peng2010}. We estimate that yield erosion has decreased the yield of
medium grain rice cultivars released by the CCRRF over the last 30+
years by an average of \Sexpr{yrs_coef} Mg ha\tsups{-1} year\tsups{-1}, potentially
offsetting any gains made by the crop improvement program. However, it
is not clear what is responsible for this erosion. It is unlikely
biotic factors as pest and disease pressure are low in this
...
for pest and disease pressure. While efforts should be made to
identify the cause of erosion in this environment, this observation
highlights both the need for maintenance of plant breeding and the
difficulty of increasing genetic yield potential, especially in
systems such as CA where yield is approaching the physiological limit
\citep{espe16a}. Without maintenance breeding (i.e., continuously
selecting cultivars well adapted to current conditions), CA rice
production systems are predicted to have lost an estimated
...
Fig. \ref{fig:ids_comp})) are not widely adopted. It is known that
cool temperatures during the booting stage can have a large impact on
grain yields~\citep{espe2017}, and the RES experiences warmer
temperatures than many other rice growing areas in CA
\citep{espe2017}. These cultivars may perform well at the RES but not
widely in CA. Therefore, it should be noted that while our
...
\citep{Foley2011a}. Given the estimated yield erosion, to hit an
...
yield. The evidence here is consistent with the observation that the
genetic yield potential has not increased substantially in the last
30+ years (Fig. \ref{fig:first_yrs}).
...
period and are important. Also, crop duration has been reduced. For
example, during this study period, M-202 and M-206 have been the
predominate cultivars grown in CA, and while their yield potentials
were similar at the time of release (Fig. 5), M-206 matures about 6
days earlier than M-202 (Table \ref{tbl:ids}). Shorter duration is very important
in CA due to an already short growing season and water limitations
(shorter duration cultivars would presumably require less irrigation
water).

California state average yields have increased slowly
despite there being little to no gains in the breeding trials
...
Pittelkow2012, linquist2008}, and other factors may all be contributing to
increases in the state average yield. However, these gains are far less
than those experienced during the preceding era, and by their
...
facing rice breeding. We suggest here that historical benchmarks be
...
\caption{The marginal estimates for the difference between cultivars,
  ...
  yield erosion and annual yield gains).}
\label{fig:ids_comp}
\end{figure}
