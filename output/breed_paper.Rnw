...
and classical approaches.  The Bayesian estimates are presented here
due to the inherent flexibility of Bayesian inference, though the
classical estimates can be found in the supplemental
...
(Fig \ref{fig:RES_year}. Evaluating these initial yields over time
indicates there was no evidence of genetic yield improvements during
the 30-year study period (Fig. \ref{fig:RES_year}). Individual exceptions to this were
two cultivars (M-204 and M-205) that had higher yields than the
others.
...
The yields of each cultivar over time were evaluated relative to when
the cultivar was released using the full model which accounts for both
yield erosion and yield improvements over time. The average decrease
in yield over time (i.e., yield erosion) once a
...
has lost an average of \Sexpr{round(yrs_coef*-36/1000,1)} Mg
...
omitted from the model), there is estimated to be a small increase per year
...
full model which corrected for both the year of release and yield
