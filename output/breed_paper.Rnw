...
m202_v_206 = round(release_yr_coef * 19, 0)
...
The Rice Experiment Station (RES) of the CCRRF is located north-west of
Biggs, CA (39.4648, -121.7342), and has been the central location for
CCRRF's efforts to develop improved rice varieties adapted for CA since
...
\caption{The relationship between yield and the years since release
  ...
  \Sexpr{yrs_coef} kg ha\textsuperscript{-1}
...
(\Sexpr{yrs_coef} kg ha\textsuperscript{-1} year\textsuperscript{-1}).}
...
When yields are compared accounting for differences in the years since
release at the time of comparision, there is a small decrease in yield
over time once a cultivar is released
(\Sexpr{yrs_coef}
kg ha\textsuperscript{-1} year\textsuperscript{-1}, 90\%
CI: \Sexpr{RES_CI[2]} -- \Sexpr{RES_CI[1]}). The oldest cultivar in
the trials is estimated to have lost \Sexpr{round(yrs_coef*36,1)}
kg ha\textsuperscript{-1} since release. Again
comparing the yield of two prominant cultivars, M-202 and M-206, the
two cultivars are experiencing a similar trend over time to eachother
(Fig. \ref{fig:compare_yrs}) and the estimate from all released medium
grains. The estimated yield advantage of the newer cultivar (M-206)
(\Sexpr{m202_v_206} kg ha\textsuperscript{-1}) can be explained by the
differences in the amount of time that has passed since each cultivar
was released (Fig. \ref{fig:compare_yrs} \&
\ref{fig:compare_since_release})
...
differences between the years since release were accounted for
(Fig. \ref{fig:release}) (\Sexpr{release_yr_coef}
kg ha\textsuperscript{-1} year\textsuperscript{-1}). This suggests
that the overall progress on yield over the period from 1980 -- 2016
is approximately \Sexpr{round(release_yr_coef * 36, 0)} kg
ha\textsuperscript{-1}.

When differences between the number of years since each cultivar's
release are taken into account, the estimated increase in grain yield
over time is estimated to be \Sexpr{release_yr_coef}
kg ha\textsuperscript{-1} year\textsuperscript{-1} (90\% CI:
\Sexpr{rel_CI[1]} -- \Sexpr{rel_CI[2]}). Therefore, the evidence is
consistent with no increase in yield potential over the study
period. If the differences in years since release of each cultivar is
ignored, there appears to be a small increase over time (approximately
\Sexpr{round(coef(lm(yield_kg ~ release_yr, mod_data[res,])), 1)})
(Fig. \ref{fig:release}), much less than previous estimates of
\Sexpr{54 * 1.12} kg ha\textsuperscript{-1} year\textsuperscript{-1}
(CITE).  Comparing the two dominate cultivars M-202, which was widely
planted from the late 1980s until the early 2000s, and M-206,
currently the most widely planted CRRFS cultivar (approximately 50\%
of planted area), there is an estimated \Sexpr{m202_v_206} kg
ha\textsuperscript{-1} yield advantage of M-206 over M-202 (Figs
\ref{fig:release} \& \ref{fig:compare_vars}), though both cultivars
yielded similarly at their respective year of release
(Fig. \ref{fig:compare_yrs}).

\section{Discussion}

\subsection{Improving yield potential over time}

Overall, we found evidence consistent with a no improvements in yield
potential in medium grain rice cultivars released by CCRRF's breeding
improvement program over the period \Sexpr{min(mod_data[,'year'])} --
\Sexpr{max(mod_data[,'year'])}. Rather, it seems possible that the
breeding program has been maintaining yield potential in the face of
changing biotic and abiotic pressures, which are expected to decrease
yield over time \citep{Peng2010}. This highlights both the need
for maintanence breeding and the difficulty of increasing genetic
yield potential as yield approaches the physiological limit. Without
maintanence breeding continuously selecting cultivars well adapted to
current conditions, CA rice production systems are predicted to have
lost an estimated \Sexpr{round(yrs_coef * 31, 0)} kg
ha\textsuperscript{-1} grain yield over the study period.

However, the goal of a breeding program is to not only recover the
benchmark yield of the old standard cultivar but to surpass it. Given
the estimated yield decreases over time, to hit an ambitious target of
500 kg ha\textsuperscript{-1} (approximately 5\%) increase in yield
per release (with roughly 5 years between releases), a new cultivar
needs to yield \Sexpr{round(yrs_coef * -5, -1)} more
in side-by-side comparison on top of the 500 kg ha\textsuperscript{-1}
target improvement. Often side-by-side yield improvements (not
considering the difference in years since release) are closer to 2 --
3\% per release, in which case no absolute improvements might be made
beyond maintenance. The evidence here is consistent with the latter
situation occuring in CA, which suggests that the genetic yield
potential has not increased substantially in the last 30+ years (Fig. \ref{fig:release}).

There has been some yield improvements over the last 30 years in the
CA state average yield (Fig. \ref{fig:state_yield}), but the evidence
here suggests mechanisms other than increased genetic yield potential
are responsible (Fig. \ref{fig:compare_yrs}). Improvement in agronomic
management, including more efficient nutrient management
\citep{Lundy2012, Lundy2015, linquist2009}, weed management
\citep{Saito2008, Pittelkow2012, linquist2008}, etc. may all be
contributing to further increases. However, the gains here are less
than those experienced during the ``Green revolution'', and by their
nature cannot surpass the genetic yield potential
\citep{Fischer2015}. Judging from yields achieved at the RES
(Figs. \ref{fig:RES_year} \& \ref{fig:compare_yrs}), a
researcher-managed production system, state average yields
(approximately 10 Mg ha\textsuperscript{-1} at the time of writing;
Fig. \ref{fig:state_yield}) may be approaching the agronomic limit.

To be clear, our investigation here is narrowly focused on the
rough-rice/field grain yield. Other improvements due to plant
improvement, including qualtity \citep{mckenzie1993} and milling yield
...
increases in grain yield \citep{Godfray2010, Foley2011a,
Mueller2012}. The evidence here suggests that to accomplish true
increases over time, the improvement target needs take decreases in
the current cultivars' yield over time into account. While side-by-side
comparisions will continue to be useful, comparisons across years to
each cultivar's benchmark yield at the time of release should provide
information on the absolute improvements being made in yield potential over time.
...
text(25, 10.75, "(3) target above maintenance", cex = 0.8,
     srt = 15)#((atan(fixef(RES_full)[3] + 0.06) * 180))/pi)
abline(h = 10, lty = 3, lwd = 2)
text(25, 10.1, "(1) current situation", cex = 0.8)
abline(a = 10, b = fixef(RES_full)[3] + 0.06, lty = 4, lwd = 2)
text(25, 9.25, "(2) no maintenance breeding", cex = 0.8,
     srt = -20)
...
\caption{A conceptual diagram highlighting three scenarios:
  ...
  where grain yield slowly decreases over time, and (3) absolute
...
cultivars rather than side-by-side comparisions to maintain
perspective of the absolute gains being made.
...
@
\caption{An example of a typical comparison between a older (M-202)
  ...
  \Sexpr{max(comp_yrs)}, approximately the comparision made to
  determine if a new cultivar performs sufficiently better. The newer
