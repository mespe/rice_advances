...
system, California (CA) rice, in order to quantify improvements
...
cultivars which do not consider yield erosion, there was an apparent
...
having a Mediterranean climate with long days and a dry growing season
...
to 4 times in a CRB design) were collected leading to a final data set
...
time. For comparison to estimates where yield erosion is not taken
...
yield erosion by adding a random slope by cultivar (Model 3).
...
(Fig \ref{fig:first_yrs}. Comparing these yields from the time of
release for each cultivar does not provide
...
\caption{The relationship between yield and the years since release
  ...
  cultivar and year) is estimated by a mixed-effects model to be
...
the rate of yield erosion (Model 3); although visual evidence suggest
otherwise (Fig. \ref{fig:all_cult}), more data is needed. Based on
this, the models excluding varying rates of erosion by cultivar (i.e.,
Models 1 \& 2) are presented here. The yields of each cultivar over time were
evaluated relative to when the cultivar was released using the full
model (Model 1) which accounts for both yield erosion and yield improvements
over time. The average decrease in yield over time (i.e., yield
erosion) once a cultivar was selected was estimated at
\Sexpr{yrs_coef} kg ha\tsups{-1} year\tsups{-1} (90\% CI:
\Sexpr{RES_CI[2]} to \Sexpr{RES_CI[1]}) (Fig \ref{fig:RES_year}; Table
\ref{tbl:coef}). The estimated 560 kg ha\tsups{-1} yield advantage of
M\=/206 over M\=/202 (the current and former predominant cultivars,
respectively), can be attributed solely to the erosion of M\=/202's
yield over time (Fig. \ref{fig:first_yrs} \&
...
released 19 years prior to M\=/206 (Table
\ref{tbl:ids}).
...
When yield erosion is not accounted for in the analysis (Model 2),
...
(Fig. \ref{fig:wo_erosion}). This analysis is functionally similar to evaluating
cultivars in side-by-side yield tests, as the differences in years
since release is between cultivars is not accounted for. However, the
full model (Model 1) which accounts for both the year of release and
...
(Fig. \ref{fig:first_yrs}). According to model comparison metrics
(PS-LOO, WAIC), the full model (Model 1) is estimated to have superior
predictive accuracy compared to the model which does not account for
yield erosion (Model 2)
...
have higher than average yields, even after correcting for yield
...
comparisons. However, when accounting for yield erosion and looking
...
plausible. This analysis suggests that the breeding program has
...
for yield erosion of the cultivar IR8 at IRRI~\citep{Peng1998,
...
if this mechanism is responsible for the yield erosion of
...
It is striking that the estimated yield erosion is similar to that
...
increasing genetic yield potential~\citep{espe2017}, especially in
...
management~\citep{Lundy2012, linquist2009}, and weed
management~\citep{Pittelkow2012, linquist2008}. However,
...
with increasing yield potential, the estimates here might not capture
...
abline(v = 0, lty = 2)
@
\caption{The marginal estimates (i.e., random effects) for the
  difference between cultivars, averaged over the uncertainty in the
  fixed effect parameters (i.e., yield erosion and annual yield gains).}
\label{fig:ids_comp}
\end{figure}
